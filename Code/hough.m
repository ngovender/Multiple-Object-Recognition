...
function [originalFeatures1]= hough(image1, image2, matches, descriptor1,descriptor2,sizeX,sizeY)
...
for i = 1:matches
    ...
    % centre of the image
    diffDistX = sizeX - image1(i,1);
    diffDistY = sizeY - image1(i,2);
    distance = [diffDistX;diffDistY]; 
    % difference in angles
    angleImage1 = refAngle - image1(i,4);
    
    scaleFactor = image1(i,3)/ image2(i,3); % use this scale to vote on scale?
     
    % Image 2
    angleImage2 = refAngle - image2(i,4); % angle used for voting
    diffAngle = (angleImage1 - angleImage2);
    % rotation matrix
    rot = [cosd(diffAngle),-sind(diffAngle);sind(diffAngle), cosd(diffAngle)];
  
    currentImagePos = [image2(i,1), image2(i,2)];
     ...
     if ((voting(i,2)>=(maxRangeY-locRange)) & (voting(i,2)<(maxRangeY)))%fulfills the locY
          if ((voting(i,3)>=(scaleRange-2)) & (voting(i,3)<scaleRange))% scale
              if ((voting(i,4)>=(numOri-60)) & (voting(i,3)<(numOri+30))) % orientation  %features satifies all criteria, should save it
                  ...
                  originalFeatures1(count).locationX = image1(i,1);
                  originalFeatures1(count).locationY = image1(i,2);
                  originalFeatures1(count).scale = image1(i,3);
                  originalFeatures1(count).orientation = image1(i,4);
                  originalFeatures1(count).descriptor = descriptor1(i,:);
                
                  %need to save the original matches 
                  originalFeatures2(count).locationX = image2(i,1);
                  originalFeatures2(count).locationY = image2(i,2);
                  originalFeatures2(count).scale = image2(i,3);
                  originalFeatures2(count).orientation = image2(i,4);
